    ...
    if source_file_extension == EXT_FILTER:
        ...
        if extraction_tmp == None:
            ...
            if CLEAN:
                ...
                oldpath, filename = os.path.split(extraction_tmp)
                dst = os.path.join(OUT_DIR, filename)
                shutil.move(extraction_tmp, dst)
                
                oldpath, filename = os.path.split(decomment_tmp)
                dst = os.path.join(OUT_DIR, filename)
                shutil.move(decomment_tmp, dst)
                
                for src in split_tmp:
                    oldpath, filename = os.path.split(src)
                    dst = os.path.join(OUT_DIR, filename)
                    shutil.move(src, dst)
...
if __name__ == "__main__":
>>>>>>> Move temp files to output folder when using -y
    main()